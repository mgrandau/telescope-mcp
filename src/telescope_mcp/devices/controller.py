...
class SyncCaptureResult:
    ...
    def timing_error_ms(self) -> float:
        ...
        Converts timing_error_us (microseconds) to milliseconds for easier reading in logs, UI displays,
        and diagnostics. Positive values indicate secondary started late, negative early. Magnitude
        indicates synchronization quality.
        
        Business context: Critical metric for assessing dual-camera synchronization quality in telescope
        operations. Platesolving workflows require finder and main camera captures taken at same moment
        (within ~100ms) to ensure both frames show same sky position. Large timing errors (>500ms) may
        cause platesolve failures if telescope drifted between captures. Used in quality monitoring
        ("are captures synchronized well enough?"), performance tuning (adjusting clock resolution,
        thread priorities), and troubleshooting ("why did alignment fail?"). Logged to session metadata
        for post-observation analysis.
        
        Implementation details: Simple conversion: timing_error_us / 1000.0. Returns float with
        sub-millisecond precision (e.g., 1.234ms). Sign preserved: positive = secondary late, negative
        = secondary early. Typical values: 0-50ms (good synchronization), 50-200ms (acceptable),
        >200ms (poor - may affect platesolving). Zero indicates perfect timing (rare - usually 1-20ms
        due to thread scheduling jitter).
        
        Returns:
            Timing error in milliseconds as float. Range typically -500.0 to +500.0 (extreme cases),
            commonly -50.0 to +50.0 for well-tuned systems. Precision to 0.001ms (1 microsecond).
...
class CameraController:
    ...
    def add_camera(self, name: str, camera: Camera) -> None:
        ...
        Business context: Enables runtime camera registration for flexible multi-camera
        setups. Supports dynamic camera addition as hardware is connected, hot-swapping
        cameras without controller restart, and descriptive naming that clarifies role
        (e.g., "main_imaging", "finder_guide", "autoguider"). Essential for observatory
        setups where camera configuration may change per observation session or target.
        
        Implementation details: Does not validate camera state or connectivity - caller
        should ensure camera is connected and configured before registration. Duplicate
        names will overwrite previous registration without warning. Consider validating
        camera.is_connected before adding in production code.
        
        Args:
            name: Unique name for the camera (e.g., "finder", "main", "autoguider").
                Used as key in sync_capture config. Should be descriptive and match
                names used elsewhere in the system for consistency.
            camera: Camera instance, typically already connected via Camera.connect().
                Should have driver initialized and be ready for capture operations.
    ...
    def sync_capture(self, config: SyncCaptureConfig) -> SyncCaptureResult:
        ...
        exposure centered within the primary exposure. Ensures both cameras capture
        the same temporal moment at their respective exposure midpoints, critical
        for accurate alignment and tracking in astrophotography.
        
        Business context: Essential for synchronized multi-camera astrophotography
        where precise temporal alignment is required. Primary use case is plate solving
        for telescope alignment: a long-exposure finder camera captures sufficient
        stars for plate solving (determining telescope pointing), while a short-exposure
        main camera captures a quick frame at the same moment for alignment verification.
        This ensures the plate solve result accurately represents where the main camera
        was pointing, accounting for atmospheric refraction, mount flexure, and tracking
        errors that vary over time.
        
        Additional use cases:
        - Autoguiding: Guide camera long exposure centered on science camera exposure
        - Multi-wavelength imaging: Synchronized captures across different filters
        ...
        cameras see the sky at the same moment despite different exposure durations.
        
        Implementation details: Uses ThreadPoolExecutor for concurrent camera control.
        Primary capture starts immediately, secondary waits for calculated delay before
        starting. Both captures block until complete. Timing measurements use monotonic
        clock to avoid wall clock discontinuities. Timing error is typically <10ms
        on modern systems but may increase under heavy system load.
        ...
        Returns:
            ...
            Use timing_error_ms < 50ms as "good" threshold for most applications.
        ...
        def capture_primary() -> None:
            """Thread worker capturing primary camera frame (typically finder/guide camera).
            
            Executes in dedicated thread capturing primary camera frame immediately without delay.
            Records start timestamps (monotonic for timing calculations, wall-clock for metadata).
            Catches all exceptions storing in primary_error for main thread handling. Uses nonlocal
            to communicate results back to sync_capture scope.
            
            Business context: Primary camera typically finder/guide (short exposure, wide field) capturing
            stars for platesolving while secondary (main imager) captures verification frame. Primary starts
            immediately establishing timing reference point. Threading enables overlap: while primary exposing
            (e.g., 2s finder exposure), secondary thread waits calculated delay then starts (e.g., 1.9s wait
            then 0.2s main exposure). Without threading, captures would be sequential losing synchronization.
            
            Implementation: Records self._clock.monotonic() for high-precision timing, datetime.now(UTC) for
            human-readable timestamps. Calls primary_cam.capture_raw() (no overlay) with configured exposure/gain.
            Exceptions stored in primary_error (checked by main thread after join). Nonlocal variables modified:
            primary_result (CaptureResult), primary_error (Exception), primary_start_mono (float), primary_start_wall
            (datetime). Thread executor ensures clean cleanup even on exception.
            """
        ...
        def capture_secondary() -> None:
            """Thread worker capturing secondary camera frame with calculated delay (typically main camera).
            
            Executes in dedicated thread sleeping for calculated delay (temporal centering: makes exposure
            midpoints align), then capturing secondary camera frame. Records start timestamps. Catches all
            exceptions storing in secondary_error. Uses nonlocal for result communication.
            
            Business context: Secondary camera typically main imager (long exposure, narrow field) capturing
            verification frame at moment finder solves. Delay ensures both cameras capture same sky position
            despite different exposure times. Example: finder 2s exposure (midpoint 1s), main 0.2s exposure
            needs 0.9s delay so midpoint aligns at 1s. Critical for platesolving accuracy - misaligned captures
            show different sky positions due to telescope drift/tracking errors, causing solve failures or
            inaccurate alignment models.
            
            Implementation: Sleeps delay_seconds (calculated by calculate_sync_timing) using self._clock.sleep(),
            then records timestamps and calls secondary_cam.capture_raw(). Sleep blocks thread without busy-waiting.
            Monotonic clock used for timing precision (immune to system clock adjustments). Exceptions stored in
            secondary_error (checked after join). Nonlocal variables: secondary_result (CaptureResult),
            secondary_error (Exception), secondary_start_mono (float), secondary_start_wall (datetime). Thread
            executor handles cleanup.
            """
