...
class PositionSensor(Protocol):
    ...
    def read(self) -> TelescopePosition:
        ...
        Example:
            ...
            >>> pos = sensor.read()
            >>> print(f"Alt={pos.altitude:.1f}°, Az={pos.azimuth:.1f}°")
            Alt=45.0°, Az=180.0°
        """
        ...
...
class StubPositionSensor:
    ...
    def __init__(self) -> None:
        ...
        Creates simulated encoder sensor returning fixed position (45° altitude,
        180° azimuth) until calibrated. No hardware communication. Position fixed
        until calibrate() called.

        Business context: Enables development of position-aware telescope features
        without physical encoders or IMU sensors. Developers test goto accuracy
        calculations, plate solving integration, position displays in UI. CI/CD
        validates positioning logic with predictable values. Critical for
        development environments lacking sensor hardware.

        Implementation details: Initializes _position as TelescopePosition(45.0,
        180.0). read() returns this value. calibrate() updates stored position.
        No drift, noise, or hardware errors. Used by DriverFactory in
        DIGITAL_TWIN mode.
